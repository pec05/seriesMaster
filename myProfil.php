...
<html>
	...
	<body> 
		...
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT nomcli from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
			...
			$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
			$_SESSION['nomcli'] = $row["nomcli"];
		?>
		<!-- PRENOM-->
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT prenomcli from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
			...
			$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
			$_SESSION['prenomcli'] = $row["prenomcli"];
		?>
		<!-- TELEPHONE-->
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT telcli from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
			...
			$row = mysqli_fetch_array($result,MYSQLI_ASSOC);
			$_SESSION['telcli'] = $row["telcli"];
		?>
		<!-- CODE POSTAL-->
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT codepostal from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
		...
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT adresse from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
		...
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT ville from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
		...
		<?php 
			$myusername = $_SESSION['login_email']; 
			$sql = "SELECT pays from user where email like '$myusername'";
			$result = mysqli_query($db,$sql);
			if (!$result) {
				printf("Error: %s\n", mysqli_error($db));
				exit();
	...
	<div class="container">
  	...
  	<?php if(isset($_SESSION['login_email'])): ?>
        ...
        <body onload ="show_p_tmdb('<?php echo $_SESSION['sessionID']?>')">
	<div class ="main_search">
		<div class="main_search_remove">
            <div class="main_mp main">
                <div class="tmdb_profil">
				</div>
				<div class="tmdb_link">
					<div class="container-fluid">
						<div class="row">
							<div class="col-sm-12">
								<div class="link">
								<a id="link_tmdb" href="https://www.themoviedb.org"><img src="css/images/408x161-powered-by-rectangle-blue-10d3d41d2a0af9ebcb85f7fb62ffb6671c15ae8ea9bc82a2c6941f223143409e.png"></a>
								</div>
							</div>
						</div>
					</div>
				...
				<a name="sm_pro"></a>
				<div class="sm_profil">
					<div class="tmdb_profil_title"><h3>My profil SeriesMaster : </h3></div>
					<div class='row'>
						<div class='col-sm-12 text-center'>
							Name : <?php echo $_SESSION['nomcli']?>
							<br>
							Firstname : <?php echo $_SESSION['prenomcli']?>
							<br>
							Phone number : <?php echo $_SESSION['telcli']?> 
							<br>
							Postal code : <?php echo $_SESSION['codepostal']?> 
							<br>
							Adress : <?php echo $_SESSION['adresse']?> 
							<br>
							City : <?php echo $_SESSION['ville']?>
							<br>
							Country : <?php echo $_SESSION['pays']?>
				</div>
            </div>    
		</div>	        
	</div>					
